                      ...
                      _In_ wchar_t *command_line, _In_ int show_command)
{
  ...
  if (SendAppLinkToInstance(L"XLink"))
  {
    return EXIT_SUCCESS;
  }

  HANDLE hMutexInstance = CreateMutex(NULL, TRUE, L"XLinkMutex");
  HWND handle = FindWindowA(NULL, "XLink");
  ...
  if (GetLastError() == ERROR_ALREADY_EXISTS)
  {
    ...
    if (window.SendAppLinkToInstance(L"XLink"))
    {
      return false;
    }
  ...
  if (!window.Create(L"XLink", origin, size))
  {
    return EXIT_FAILURE;
  }
